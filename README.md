...
它的主要调用者是 AI Agent（智能体），不是要求人长期打开 OpenCode、CodeG 或终端逐条执行命令。AI 应先读取结构化状态，再按失败关闭合同选择状态、备份、恢复、桥接或更新入口，并把可核验回执解释给人；人只在外部发消息、付费模型调用、版本更新、常驻修复或灾备激活等明确边界上授权。CodeG/Cline 只是可选客户端之一，不是产品成立的前提。
